...
fig, chart = plt.subplots(facecolor=(0,0,0))
chart.tick_params(labelcolor=(1,0,0))
chart.set_facecolor((0,0,0))
chart.plot(x,proc_spec)
chart.set_xlabel('Raman Shift (cm$^{-1}$)',color=(1,0,0))
chart.set_ylabel('Normalized Intensity',color=(1,0,0))
bkg_plotvar = 0
drk_plotvar = 0
exp_plotvar = 0
...
while True:
    ...
    if len(drk_scan)>0 and len(bkg_scan)>0 and len(exp_scan)>0:
        ...
        plt.clf()
        if drk_plotvar==0:
            chart.plot(range(0,len(drk_scan)),[((drk_i-min(drk_scan))/(max(drk_scan)-min(drk_scan)))*drk_scale for drk_i in drk_scan],label='drk_scan')
        if bkg_plotvar ==0:
            chart.plot(range(0,len(bkg_scan)),[((bkg_i-min(bkg_scan))/(max(bkg_scan)-min(bkg_scan)))*bkg_scale for bkg_i in bkg_scan],label='bkg_scan')
        if exp_plotvar == 0:
            chart.plot(range(0,len(exp_scan)),[((exp_i-min(exp_scan))/(max(exp_scan)-min(exp_scan)))*exp_scale for exp_i in exp_scan],label='exp_scan')
        chart.plot(x, proc_spec, label='proc_spec')
        chart.legend(bbox_to_anchor=(1.05,1),loc=2, borderaxespad=0.)
        chart.tick_params(labelcolor=(1,0,0))
        chart.set_facecolor((0,0,0))
        chart.plot(x,proc_spec)
        chart.set_xlabel('Raman Shift (cm$^{-1}$)',color=(1,0,0))
